...
function Tracks() {
  ...
  useEffect(() => {
    axios.get(api_url)
      .then((response) => {
        // Simulate track data, adjust mapping based on your API response
        const trackData = response.data.map(item => ({
          id: item.id,
          title: item.title === "Front-End" ? "Front-End" : "Data Analytics", // Example mapping
          image: item.title === "Front-End" ? "https://via.placeholder.com/120?text=Front-End" : "https://via.placeholder.com/120?text=Data+Analytics" // Placeholder images
        }));
        setTracks(trackData.slice(0, 2)); // Limit to 2 tracks for this example
      })
  ...
  return (
    <>
      ...
      <Row>
        {tracks.map((track) => (
          <Card key={track.id} className="card-track">
            ...
            <Card.Img className="card_img" variant="top" src={track.image} />
            <Link id="link-det-track" to={`/track-courses/${track.id}`}>
              <Card.Body>
                <Card.Title className="card_title">{track.title}</Card.Title>
              </Card.Body>
            </Link>
          </Card>
        ))}
