...
function My_learning() {
  ...
  useEffect(() => {
    axios.get(api_url)
      .then((response) => {
        // Simulate course data, adjust mapping based on your API response
        const courseData = response.data.map(item => ({
          ...
          nextVideo: {
            title: "C++ OOP - What is polymorphism in programming?",
            duration: "10 min"
          }
        }));
    ...
    <>
      ...
      {courses.map((course) => (
        <Row key={course.id} className="bordd">
          <Col><Card.Img className="card_img1" variant="top" src={course.image} /></Col>
          <Col xs={6} className="m-auto content-progress">
            <Card.Body>
              <Card.Title className="card_title1">{course.title}</Card.Title>
              <input type="range" value={course.progress} readOnly />
              <p id="over-progress">Overall Progress</p>
            </Card.Body>
          </Col>
          <Col className="content-next">
          ...
          <Col className="content-next">
            ...
            <p id="txt-nxt-video">{course.nextVideo.title}</p>
            <p id="time-video">Video . {course.nextVideo.duration}</p>
          </Col>
        </Row>
      ))}
    </>
